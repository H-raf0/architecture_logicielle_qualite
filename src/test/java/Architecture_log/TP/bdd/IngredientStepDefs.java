...
public class IngredientStepDefs {
  ...
  public void recetteContientIngredientsTableau(
    int recetteId,
    DataTable dataTable
  ) {
    ...
    List<Map<String, String>> ingredients = dataTable.asMaps(
      String.class,
      String.class
    );
  ...
  @Étantdonné(
    "que la recette {int} contient un ingrédient {string} avec l'id {int}"
  )
  public void recetteContientIngredientAvecId(
    int recetteId,
    String nom,
    int ingredientId
  ) {
  ...
  @Étantdonné(
    "que la recette {int} contient un ingrédient {string} avec {string} et l'id {int}"
  )
  public void recetteContientIngredientComplet(
    int recetteId,
    String nom,
    String quantite,
    int ingredientId
  ...
  // Action steps
  @Quand(
    "j'ajoute un ingrédient {string} avec la quantité {string} à la recette {int}"
  )
  public void ajouteIngredient(String nom, String quantite, int recetteId)
    throws Exception {
    ...
    lastResult = mockMvc
      .perform(
        post("/api/recettes/{recetteId}/ingredients", recetteId)
  ...
  public void recupereIngredientsRecette(int recetteId) throws Exception {
    lastResult = mockMvc
      .perform(get("/api/recettes/{recetteId}/ingredients", recetteId))
      .andReturn();
    lastStatusCode = lastResult.getResponse().getStatus();
  }

  @Quand("je récupère l'ingrédient {int} de la recette {int}")
  public void recupereIngredientSpecifique(int ingredientId, int recetteId)
    throws Exception {
    lastResult = mockMvc
      .perform(
        get(
          "/api/recettes/{recetteId}/ingredients/{ingredientId}",
          recetteId,
          ingredientId
        )
      )
  ...
  public void modifieIngredient(int ingredientId, String quantite)
    throws Exception {
    Ingredient ingredient = ingredientRepository
      .findById((long) ingredientId)
      .orElse(null);
    ...
    lastResult = mockMvc
      .perform(
        put("/api/ingredients/{ingredientId}", ingredientId)
          .contentType("application/json")
          .content(json)
      )
      .andReturn();
  ...
  public void supprimeIngredient(int ingredientId, int recetteId)
    throws Exception {
    lastResult = mockMvc
      .perform(
        delete(
  ...
  public void recetteContientIngredient(int recetteId, String nom) {
    ...
    List<Ingredient> ingredients = ingredientRepository.findByRecetteId(
      (long) recetteId
    );
  ...
  public void listeContientIngredient(String nom, String quantite)
    throws Exception {
    String content = lastResult.getResponse().getContentAsString();
